...
<head>
    ...
    <!-- boardTemplate.jsp의 head 내 script 태그 추가 -->

    <script>
        function goToWrite() {
            window.location.href = "${fn:toLowerCase(fn:replace(board.boardName, ' ', ''))}-write.action";
        }
        ...
        $(document).ready(function() {
            ...
            var boardNameUrl = 'board' + boardName.toLowerCase().replace(/\s/g, '');
            ...
            $('.board-row').each(function() {
                var postIdLink = $(this).find('a[href*="-post.action"]').attr('href');
                if (postIdLink) {
                    var postId = postIdLink.split('postId=')[1];
            ...
            function loadPosts(page, hotOnly) {
                ...
                var boardId = ${not empty board.boardId ? board.boardId : 0};

                $.ajax({
                    url: '/api/' + boardNameUrl + '.action',
                    type: 'GET',
                    data: {
                        page: page,
                        size: 10,
                    ...
                    success: function(response) {
                        ...
                        var newUrl = boardNameUrl + '.action?page=' + page;
                        if (hotOnly) {
                            newUrl += '&hotOnly=true';
                        }
                        if (searchKeyword) {
            ...
            function updateTable(data) {
                ...
                // 인기글 표시 (인기글 모드가 아닐 때만 상단에 표시)
                if (!data.hotOnly) {
                    $.each(data.hotPost, function(index, post) {
                        html += '<tr class="board-row hot-post border-bottom">' +
                            ...
                            '<td class="p-3 title-cell"><a href="' + boardNameUrl + '-post.action?postId=' + post.postId + '">' + post.postTitle + '</a></td>' +
                ...
                // 게시글이 없을 경우
                if (data.notice.length === 0 && (data.hotOnly || data.hotPost.length === 0) && (!data.postList || data.postList.length === 0)) {
            ...
            function getCategoryClass(labelName) {
                ...
                if (labelName === '공지') return 'notice';
                if (labelName === '고독한캠핑') return 'solocamping';
                return '';
            }
        ...
        <div class="main-content d-flex gap-4 my-5">
            ...
            <div class="main-column" style="flex: 1; padding-left: 5px;">
                <div class="page-header">
                    <h1 class="page-title">
                        <i class="fa-solid fa-${board.boardName == '자유 게시판' ? 'comments' :
                                                'folder'}"></i>${board.boardName}
                    </h1>
                </div>
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div style="display: flex; margin-left: 20px;">
                            <a href="#" class="filter-btn active"
                               style="border-radius: 4px; margin-right: 10px; min-width: 80px; text-align: center; background-color: var(--color-beige-light) !important; border-color: var(--color-maple) !important; color: var(--color-maple) !important; font-weight: var(--font-semibold) !important;">전체</a>
                    ...
                    <table class="w-100">
                        ...
                        <c:forEach var="hotPost" items="${hotPost}">
                            <tr class="board-row hot-post border-bottom">
                                ...
                                <td class="p-3 title-cell">
                                    <a href="boardfree-post.action?postId=${hotPost.postId}">${hotPost.postTitle}</a>
                                </td>
                        ...
                        <c:forEach var="postList" items="${postList}" varStatus="status">
                            <tr class="board-row border-bottom">
                                ...
                                <td class="p-3 title-cell">
                                    <a href="board${fn:toLowerCase(fn:replace(board.boardName, ' ', ''))}-post.action?postId=${postList.postId}">
                                            ${postList.postTitle}
                                    </a>
                                </td>
                ...
                <div style="display: flex; align-items: center; margin-top: 30px; width: 100%;">
                    ...
                    <div style="width: 240px; position: relative; z-index: 1; flex: 1;">
                        <form action="board${board.boardName}.action" method="get">
