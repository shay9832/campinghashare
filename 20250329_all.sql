...
INSERT INTO INSPEC_RESULT_ACTION_TYPE (INSPEC_RESULT_ACTION_TYPE_ID, INSPEC_RESULT_ACTION_TYPE_NAME) 
VALUES (INSPEC_RESULT_ACTION_TYPE_SEQ.NEXTVAL, '렌탈 입고');
INSERT INTO INSPEC_RESULT_ACTION_TYPE (INSPEC_RESULT_ACTION_TYPE_ID, INSPEC_RESULT_ACTION_TYPE_NAME) 
...
VALUES('USER1', 2, 1, '1234', '고길동', '010-0000-0001', 'user1@test.com', 1, TO_DATE('2025-02-01', 'YYYY-MM-DD'));
INSERT INTO USERS(USER_ID, USER_CODE, SIGNUP_TYPE_ID, USER_PW, USER_NAME, USER_TEL, USER_EMAIL, CREATED_DATE)
VALUES('USER2', 3, 2, '1234', '고둘리', '010-0000-0002', 'user2@test.com', TO_DATE('2025-02-08', 'YYYY-MM-DD'));
...
VALUES(RENTAL_REPLY_SEQ.NEXTVAL, 1, 2, '개인렌탈 관심 있습니다.', TO_DATE('2025-02-11', 'YYYY-MM-DD'));
INSERT INTO RENTAL_REPLY(RENTAL_REPLY_ID, RENTAL_ROOT_REPLY_ID, RENTAL_ID, USER_CODE, RENTAL_REPLY_CONTENT, CREATED_DATE)
VALUES(RENTAL_REPLY_SEQ.NEXTVAL, 1, 1, 3, '좋습니다 매칭 승인할게요', TO_DATE('2025-02-12', 'YYYY-MM-DD'));
...
-- 4보관
INSERT INTO INSPEC_RESULT_ACTION(INSPEC_RESULT_ACTION_ID, INSPEC_RESULT_ID, INSPEC_RESULT_ACTION_TYPE_ID, COMPLETED_DATE)
VALUES(INSPEC_RESULT_ACTION_SEQ.NEXTVAL, 4, 1, TO_DATE('2025-02-15', 'YYYY-MM-DD'));
...
--○ STOREN_MATCHING_REQ
-- 1스토렌
INSERT INTO STOREN_MATCHING_REQ(STOREN_MATCHING_REQ_ID
, STOREN_MATCHING_REQ_USER_ID, RENTAL_START_DATE, RENTAL_END_DATE, REQUESTED_DATE)
VALUES(STOREN_MATCHING_REQ_SEQ.NEXTVAL, 3, TO_DATE('2025-04-01', 'YYYY-MM-DD')
, TO_DATE('2025-04-07', 'YYYY-MM-DD'), TO_DATE('2025-03-20', 'YYYY-MM-DD'));
-- 2스토렌
INSERT INTO STOREN_MATCHING_REQ(STOREN_MATCHING_REQ_ID
, STOREN_MATCHING_REQ_USER_ID, RENTAL_START_DATE, RENTAL_END_DATE, REQUESTED_DATE)
VALUES(STOREN_MATCHING_REQ_SEQ.NEXTVAL, 3, TO_DATE('2025-04-10', 'YYYY-MM-DD')
...
SELECT
    STOREN_IRA_SEQ.NEXTVAL,
    IRA.INSPEC_RESULT_ACTION_ID,
    S.STOREN_ID
FROM STOREN S
JOIN PAY P ON S.STOREN_ID = P.STOREN_ID
JOIN PLATFORM_DELIVERY PD ON PD.PAY_ID = P.PAY_ID
JOIN INSPEC_RESULT IR ON IR.PLATFORM_DELIVERY_ID = PD.PLATFORM_DELIVERY_ID
JOIN INSPEC_RESULT_ACTION IRA ON IRA.INSPEC_RESULT_ID = IR.INSPEC_RESULT_ID;
...
-- 1-1스토렌(STOREN_IRA 테이블에 등록)
-- 중간 테이블에 새로운 STOREN과 기존 IRA 연결
INSERT INTO STOREN_IRA (
    STOREN_IRA_ID,
    INSPEC_RESULT_ACTION_ID,
    STOREN_ID
...
VALUES (
    STOREN_IRA_SEQ.NEXTVAL,  -- 새 ID
    :v_ira_id,               -- 기존 IRA_ID
    :v_new_storen_id         -- 새 STOREN_ID
);
...
--○ STOREN_MATCHING_REQ
-- 2-1스토렌
INSERT INTO STOREN_MATCHING_REQ(STOREN_MATCHING_REQ_ID, INSPEC_RESULT_ACTION_ID
, STOREN_MATCHING_REQ_USER_ID, RENTAL_START_DATE, RENTAL_END_DATE, REQUESTED_DATE)
VALUES(STOREN_MATCHING_REQ_SEQ.NEXTVAL, 2, 3, TO_DATE('2025-05-01', 'YYYY-MM-DD')
...
INSERT INTO STOREN
(STOREN_ID, EQUIP_CODE, SIZE_ID, STORE_MONTH, CREATED_DATE, STOREN_TITLE
, STOREN_CONTENT, DAILY_RENT_PRICE)
SELECT 
...
JOIN INSPEC_RESULT IR ON IR.PLATFORM_DELIVERY_ID = PD.PLATFORM_DELIVERY_ID
JOIN INSPEC_RESULT_ACTION IRA ON IRA.INSPEC_RESULT_ID = IR.INSPEC_RESULT_ID
JOIN STOREN_MATCHING_REQ SMR ON SMR.INSPEC_RESULT_ACTION_ID = IRA.INSPEC_RESULT_ACTION_ID
...
--○ PAY
--★ 주의사항 ★
--   : INSPEC_RESULT_ACTION_ID 1개를 여러개의 STOREN_MATCHING_REQ가 참조해서 다중 행 반환함)
--     → 가장 최근 매칭 완료 건(1건)만 반환하도록 조건 추가

-- 1스토렌(렌탈비)
INSERT INTO PAY (
    PAY_ID,                    -- 결제 ID (시퀀스 사용)
    PAY_METHOD_ID,             -- 결제 수단 ID
    STOREN_MATCHING_DONE_ID,   -- 매칭 완료 ID
    PAY_AMOUNT,                -- 결제 금액
    PAY_DATE                   -- 결제 일자
)
SELECT 
    ...
    (SMR.RENTAL_END_DATE - SMR.RENTAL_START_DATE) * S2.DAILY_RENT_PRICE,
...
JOIN PAY P2 ON PD.PAY_ID = P2.PAY_ID
JOIN STOREN S2 ON P2.STOREN_ID = S2.STOREN_ID
LEFT JOIN PAY EXISTING_PAY ON EXISTING_PAY.STOREN_MATCHING_DONE_ID = SMD.STOREN_MATCHING_DONE_ID
WHERE SMR.INSPEC_RESULT_ACTION_ID = 1
    AND EXISTING_PAY.STOREN_MATCHING_DONE_ID IS NULL
    AND ROWNUM = 1;
...
JOIN STOREN S2 ON P2.STOREN_ID = S2.STOREN_ID
LEFT JOIN PAY EXISTING_PAY ON EXISTING_PAY.STOREN_MATCHING_DONE_ID = SMD.STOREN_MATCHING_DONE_ID
WHERE SMR.INSPEC_RESULT_ACTION_ID = 2
    AND EXISTING_PAY.STOREN_MATCHING_DONE_ID IS NULL
    AND ROWNUM = 1;  -- 최신 1건만 삽입
...
(RENTER_ADD_PAY_ID, RENTER_PROBLEM_ID, RENTER_ADD_PAY_AMOUNT, PAY_DATE)
VALUES (RENTER_ADD_PAY_SEQ.NEXTVAL, 1,
    (
    SELECT ROUND(ER.ORIGINAL_PRICE * 0.2 * (IR_RETURN.EQUIP_GRADE_ID - IR_PREV.EQUIP_GRADE_ID))
    ...
    JOIN INSPEC_RESULT_ACTION IRA_PREV
        ON IRA_PREV.INSPEC_RESULT_ACTION_ID = SMR.INSPEC_RESULT_ACTION_ID
...
LEFT JOIN NICKNAME_LOG NL ON NL.USER_CODE = UC.USER_CODE
    AND NL.LAST_UPDATED_DATE = (
...
SELECT
    ...
    SLR.DELIVERY_START_DATE,
    SLR.DELIVERY_END_DATE,
    SLR.STORAGE_ID,
    NULL AS 스토렌 ID,
    NULL AS 렌탈 매칭 완료 ID,
    IRA.COMPLETED_DATE,
...
SELECT
    ...
    STLR.DELIVERY_START_DATE,
    STLR.DELIVERY_END_DATE,
    NULL,
    STLR.STOREN_ID,
    NULL,
    IRA.COMPLETED_DATE,
...
SELECT
    PL.POINT_LOG_ID AS 포인트 로그 ID,
    UC.USER_CODE AS 회원 코드,
    NL.NICKNAME AS 회원 닉네임,
    PCT.POINT_CHANGE_TYPE_NAME AS 포인트 변동 유형,
    PL.POINT_CHANGE AS 변동 포인트,
    PL.CREATED_DATE AS 변동 포인트 로그 생성일,

    EE.EVENT_ENTRY_ID AS 이벤트 응모_ID,
    EE.USED_POINT AS 이벤트 응모 포인트,

    PR.POST_REPORT_ID AS 게시글 신고 접수 ID,
    PRAT.REPORT_ACTION_TYPE_NAME AS 게시글 신고 처리 유형,

    RR.REPLY_REPORT_ID AS 댓글 신고 접수 ID,
    RRAT.REPORT_ACTION_TYPE_NAME AS 댓글 신고 처리 유형 이름,

    RRR.RENTAL_REPLY_REPORT_ID AS 렌탈 댓글 신고 접수 ID,
    RRRAT.REPORT_ACTION_TYPE_NAME AS 렌탈댓글 신고 처리 유형 이름
...
WITH BASE_RENTAL AS (
    SELECT
        R.RENTAL_ID,
        R.CREATED_DATE AS 렌탈 생성일,
        COALESCE(BL.BOOST_LOG_DATE, R.CREATED_DATE) AS 끌어올리기 일자
    FROM RENTAL R
...
BASE_STOREN AS (
    SELECT
        S.STOREN_ID,
        S.CREATED_DATE AS 스토렌 생성일,
        COALESCE(BL.BOOST_LOG_DATE, S.CREATED_DATE) AS 끌어올리기 일자,
        SMR.INSPEC_RESULT_ACTION_ID
    FROM STOREN S
    LEFT JOIN BOOST_LOG BL ON S.STOREN_ID = BL.STOREN_ID
    LEFT JOIN STOREN_MATCHING_REQ SMR ON SMR.STOREN_MATCHING_REQ_ID = (
        SELECT SMD.STOREN_MATCHING_REQ_ID
        FROM STOREN_MATCHING_DONE SMD
        JOIN PAY P ON P.STOREN_MATCHING_DONE_ID = SMD.STOREN_MATCHING_DONE_ID
...
SELECT
    ...
    I.검수 결과 처리 유형
FROM BASE_RENTAL R
FULL OUTER JOIN BASE_STOREN S
    ON R.RENTAL_ID IS NOT NULL AND S.STOREN_ID IS NOT NULL
LEFT JOIN INSPEC_INFO I
    ON S.INSPEC_RESULT_ACTION_ID = I.INSPEC_RESULT_ACTION_ID;
